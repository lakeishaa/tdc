// === TWO-MIC VARIABLE FONT — "same drugs" ONLY =============================
// Visual:
//   - Only scattered letters made from "same drugs"
//   - Always: 4 base letters "s a m e" at fixed positions
//   - Extra: up to 62 extra letters from "same drugs", scattered with
//            collision-avoidance
//
// Mic A controls:
...
async function startStreams() {
  ...
  try {
    ...
    statusSpan.html('  streaming… mic a → extra count + scale, mic b → weight');
    loop();
  } catch (e) {
    console.error(e);
...
function draw() {
  ...
  // Map audio → design
  updateMicADesign(rmsA);   // extra letters + scale
  updateMicBWeight(rmsB);   // font weight only
}
